...
public class CommentService {
    ...
    public CommentResponseDto createForInquiry(Long loginUserId, UUID inquiryId, CommentCreateRequestDto req) {
        User user = userRepository.getReferenceById(loginUserId);
        Inquiry inquiry = inquiryRepository.findById(inquiryId).orElseThrow(() -> new IllegalArgumentException("문의가 없어요."));

        Comment parent = (req.parentId() == null) ? null : commentRepository.getReferenceById(req.parentId());

        Comment saved = commentRepository.save(Comment.forInquiry(req.content(), user, inquiry, parent));
        return CommentResponseDto.of(saved);
    }
