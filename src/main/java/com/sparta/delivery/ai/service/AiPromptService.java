...
public class AiPromptService {
    ...
    public AiAnswerResponseDto GetAiWithKeywords(Long userId, AiKeywordsRequestDto requestDto) {
        ...
        if(imageCategory.equals(ImageCategory.restaurant)) {
            RestaurantKeywords keywords = (RestaurantKeywords) requestDto.keywords();
            String question = "나는 음식점 사장이야. 내 음식점의 이름은 " + keywords.name() + " 이고, " +
                    "내 음식점의 대표 메뉴는 " + keywords.mainDish() + " 야. " +
                    "내 음식점의 장점은 " + keywords.advantage() + " 이지. " +
                    keywords.highlight() + "한다는 점을 중점으로 이 음식점을 홍보하기 위한 한줄 설명 문구를 만들어줘. 단 버전은 하나로 제한해줘.";
            return aiApiService.generateAiAnswer(userId, requestDto.category(), requestDto.categoryId(), question);
        } else if (ImageCategory.valueOf(requestDto.category()).equals(ImageCategory.menu)) {
            MenuKeywords keywords = (MenuKeywords) requestDto.keywords();
            String question = "나는 음식점 사장이야. 이 메뉴의 이름은 " + keywords.name() + " 이고,  " +
                    "음식 종류는 " + keywords.category() + " 이고. " +
                    "주재료는 " + keywords.mainIngredient() + " 이야, " +
                    keywords.highlight() + "한다는 점을 중점으로 이 메뉴를 홍보하기 위한 한줄 설명 문구를 만들어줘. 단 버전은 하나로 제한해줘.";
            return aiApiService.generateAiAnswer(userId, requestDto.category(), requestDto.categoryId(), question);
        } else {
